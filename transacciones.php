...
if(!$tipo_web):
	//Gastos
	$sql="SELECT id_gasto, proveedor, metodo_pago,cuenta_gasto, fecha_gasto, monto, referencia,id_cuenta_receptora  FROM books_gastos
	...
	while($datos=mysql_fetch_assoc($q)):
		...
		$operaciones[] = array($id,$proveedor,$metodo_pago,$cuenta,$fecha,$ingreso,$egreso,$referencia,$cliente,$tipo,$id_cuenta,$id_consulta);
	endwhile;
	
	
	//Ingresos
	$sql="SELECT books_ingresos.*, books_clientes.cliente,books_metodo_pago.metodo_pago,books_tipos_ingreso.cuenta_ingreso,id_cuenta_emisora,id_consulta FROM books_ingresos 
...
function eliminaOperacion(id,tipo,id_consulta){
	if(id_consulta==0){
		...
		var mensaje = "Al eliminar este movimiento se eliminara la consulta";
	}
	swal({
