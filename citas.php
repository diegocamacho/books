...
$sql="SELECT usuarios.nombre AS doctor, pacientes.nombre, pacientes.email, pacientes.telefono, clinicas.clinica, citas.* FROM citas 
...
WHERE pacientes.activo=1 AND $tipo $consulta $rangos ORDER BY citas.fecha_hora ASC $limite";
$q=mysql_query($sql);
$citas = array();
					    ...
					    <tbody>
					      <? foreach($citas as $cita): ?>
					        <tr class="tr_<?=$cita->id_cita?>">
								...
								<td><?=$cita->clinica;?> <?if($cita->doctor):?> <br><?=$cita->doctor?> <?endif;?></td>
								<? if($_GET['tipo']!=3): ?>
									<td><? if($cita->confirmada==1): echo "<label class='font-green-jungle'>Confirmada</label>"; else: echo "<label class='font-red-thunderbird'>Pendiente por confirmar</label>"; endif;?></td>
									<td align="right">
										<? if($cita->activo==1): ?>
										<a role="button" href="?Modulo=Consulta&id=<?=$cita->id_cita?>" class="btn btn-xs green btn_<?=$ft['id_usuario']?>">Atender</a>&nbsp;
										<div class="btn-group btn_<?=$cita->id_cita?>">
								...
								<td>
									...
									<? 
										...
										$sql="SELECT encuestado FROM consultas WHERE id_cita=$id_cita";
										$q=mysql_query($sql);
										$ft=mysql_fetch_assoc($q);
										$encuestado=$ft['encuestado'];
										if($encuestado==0):
									?>
											<a role="button" class="btn btn-xs dark" onclick="javascript:encuestar(<?=$cita->id_cita?>)">Encuesta</a>
									<?	endif; ?>
								</td>
								<? endif; ?>
					        </tr>
					...
					<div class="form-group">
						...
						<div class="col-md-9">
							<div class="input-group date form_meridian_datetime_2">
                                <input type="text" size="16" class="form-control" id="fecha_final">
                                <span class="input-group-btn">
                                    <button class="btn default date-reset" type="button">
                                        <i class="fa fa-times"></i>
                                    </button>
